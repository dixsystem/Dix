...
export function DixKontrolPanel({ onClose }: { onClose: () => void }) {
  ...
  async function checkForeground() {
    ...
    try {
      ...
      setForeground(ctx);
      pushLog(ctx.supported ? `App en primer plano: ${ctx.app_name ?? "desconocida"}` : t("dix_kontrol_panel_unsupported_log"));
    } catch (e) {
  ...
  return (
    <div className="card" style={{ marginBottom: 16, overflow: "hidden" }}>
      <div style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: C.muted }}>{t("dix_kontrol_panel_title")}</span>
        <button className="btn-secondary" onClick={onClose} style={{ fontSize: 11 }}>{t("dix_kontrol_panel_close_button")}</button>
      </div>

      <div style={{ padding: "10px 16px", fontSize: 11, color: C.muted, borderBottom: `1px solid ${C.border}` }}>
        {t("dix_kontrol_panel_description")}
      </div>

      <div style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button className="btn-secondary" disabled={busy} onClick={checkForeground} style={{ fontSize: 12 }}>
            {t("dix_kontrol_panel_check_foreground_button")}
          </button>
          <span style={{ fontSize: 12, color: foreground?.supported ? C.text : C.muted }}>
            {foreground === null ? "—" : foreground.supported ? foreground.app_name ?? t("dix_kontrol_panel_unknown_app") : t("dix_kontrol_panel_unsupported_environment")}
          </span>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {!sessionActive ? (
            <button className="btn-secondary" disabled={busy} onClick={startSession} style={{ fontSize: 12, color: C.green, borderColor: `${C.green}55` }}>
              {t("dix_kontrol_panel_start_session_button")}
            </button>
          ) : (
            <>
              <span style={{ fontSize: 11, color: C.green, fontWeight: 700 }}>{t("dix_kontrol_panel_session_active")}</span>
              <button className="btn-secondary" disabled={busy} onClick={stopSession} style={{ fontSize: 12, color: C.red, borderColor: `${C.red}55` }}>
                {t("dix_kontrol_panel_stop_session_button")}
              </button>
            </>
          )}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, opacity: sessionActive ? 1 : 0.4 }}>
          <label style={{ fontSize: 12, color: C.muted }}>{t("dix_kontrol_panel_swappiness_label")}</label>
          <input
          ...
          <button className="btn-secondary" disabled={!sessionActive || busy} onClick={applySwappiness} style={{ fontSize: 12 }}>
            {t("dix_kontrol_panel_apply_change_button")}
          </button>
        </div>
